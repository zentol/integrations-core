    ...
    def submit_perf_metrics(self, instance, container_tags, container_id, container_stats):
        try:
            ...
            if system_delta > 0 and cpu_delta > 0 and active_cpus > 0:
                ...
                if cpu_percent > 500:
                    self.log.debug("Anomalous CPU percentage detected for container_id: %s. system_delta: %f, cpu_delta: %f, active_cpus: %f; prevalue_system: %f, value_system: %f, prevalue_total: %f, value_total: %f",
                        container_id, system_delta, cpu_delta, active_cpus, prevalue_system, value_system, prevalue_total, value_total)
